...
@Mock(Equipo)
class EquipoControllerUpdateSpec extends Specification {
    ...
    def "test not found"() {
        ...
        when:
            def control = mockEquipoService( true, 0 )
    ...
    private String getTemplate() {
        '<g:if test="${equipoInstance && edit}">OK</g:if><g:else>ERROR</g:else>'
    }

    private GrailsMock mockEquipoService( update = true, updateTimes = 1 ) {

        def control = mockFor( EquipoService )
        control.demand.get( 1 ) { Long id ->
            Equipo.findById( id )
        }
        control.demand.update( updateTimes ) { Equipo instance ->
            if ( update ) {
